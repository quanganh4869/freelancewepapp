...
export const RequestDetailModal = ({ request, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn font-sans">
      <div
        className="bg-studio-900 border border-slate-800 rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl relative text-slate-100"
      ...
      >
        ...
        <div className="bg-studio-950 px-6 py-4 border-b border-slate-800 flex items-center justify-between sticky top-0 z-10">
          <div>
            <div className="flex items-center gap-2">
              <span className="font-mono text-xs font-bold text-brand-primary bg-brand-primary/10 border border-brand-primary/20 px-2.5 py-0.5 rounded">
                {request.id}
              </span>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="p-4 rounded-xl bg-studio-950 border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <p className="text-xs font-mono text-slate-400 uppercase">Trạng thái xử lý</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
            ...
            <div className="p-3.5 rounded-lg bg-studio-950 border border-slate-800 space-y-1">
              <span className="text-slate-500 font-mono">Công Nghệ Yêu Cầu:</span>
              <p className="font-semibold text-slate-300">{request.preferredTechnologies || 'Để Nexus tư vấn'}</p>
            </div>
          </div>
          ...
          {(request.businessSector || request.projectStage) && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
              {request.businessSector && (
                <div className="p-3.5 rounded-lg bg-studio-950 border border-slate-800 space-y-1">
                  <span className="text-slate-500 font-mono">Lĩnh Vực Hoạt Động:</span>
                  <p className="font-semibold text-slate-200">{request.businessSector}</p>
                </div>
              )}
              {request.projectStage && (
                <div className="p-3.5 rounded-lg bg-studio-950 border border-slate-800 space-y-1">
                  <span className="text-slate-500 font-mono">Giai Đoạn Chuẩn Bị:</span>
          ...
          <div className="space-y-2">
            <h4 className="text-xs font-mono font-bold text-slate-400 uppercase">Mô tả bài toán & Mục tiêu</h4>
          ...
          {request.selectedFeatures && request.selectedFeatures.length > 0 && (
            <div className="space-y-2">
              <h4 className="text-xs font-mono font-bold text-slate-400 uppercase">Các tính năng cốt lõi đã chọn ({request.selectedFeatures.length})</h4>
          ...
          {request.internalNotes && (
            <div className="p-4 rounded-xl bg-brand-primary/10 border border-brand-primary/20 space-y-1">
              <div className="flex items-center gap-2 text-brand-primary font-mono text-xs font-bold">
                <MessageSquare size={14} /> Ghi chú từ Nexus Studio Admin:
              </div>
              <p className="text-xs text-slate-200">{request.internalNotes}</p>
            </div>
          )}
